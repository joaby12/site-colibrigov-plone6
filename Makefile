...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t https://github.com/joaby12/site_plone6-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t https://github.com/joaby12/site_plone6-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name site_plone6-backend-acceptance -d https://github.com/joaby12/site_plone6-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name site_plone6-frontend-acceptance --link site_plone6-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d https://github.com/joaby12/site_plone6-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop site_plone6-frontend-acceptance
	@docker stop site_plone6-backend-acceptance
