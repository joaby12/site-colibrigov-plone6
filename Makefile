...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t joaby12/plone-site-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t joaby12/plone-site-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name plone-site-backend-acceptance -d joaby12/plone-site-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name plone-site-frontend-acceptance --link plone-site-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d joaby12/plone-site-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop plone-site-frontend-acceptance
	@docker stop plone-site-backend-acceptance
